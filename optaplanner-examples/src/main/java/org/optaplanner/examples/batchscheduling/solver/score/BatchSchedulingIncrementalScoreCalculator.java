...
public class BatchSchedulingIncrementalScoreCalculator implements IncrementalScoreCalculator<Schedule, BendableLongScore> {
    ...
    private void insert(AllocationPath allocationPath) {
                        ...
                        if (mainDelay == null) {
                            ...
                            for (Map.Entry<Long, Segment> entry2 : segmentMap.entrySet()) {
                                if ((allocationDelayMap.get(entry2.getKey()) != null)
                                        && (entry2.getValue().getBatch().getId() != allocationPath.getBatch().getId())
                                        && (batchRoutePathMap.get(entry2.getValue().getBatch().getId()) != null)
                                        && (entry2.getValue().getRoutePath().getPath()
                                                .equals(batchRoutePathMap.get(entry2.getValue().getBatch().getId())))
                                        && (entry2.getValue().getName().equals(mainSegment.getName()))
                                        && (allocationStartTimeMap1.get(entry1.getKey()) != null)
                                        && (allocationEndTimeMap1.get(entry1.getKey()) != null)) {
                                    //String otherKey = entry2.getKey();
                                    Long newOverlapPenaltyValue = 0L;

                                    if ((allocationStartTimeMap1.get(entry2.getKey()) <= mainStartTime1)
                                            && (allocationEndTimeMap1.get(entry2.getKey()) >= mainEndTime1)) {
                                        newOverlapPenaltyValue = mainEndTime1 - mainStartTime1;
                                    } else if ((allocationStartTimeMap1.get(entry2.getKey()) >= mainStartTime1)
                                            && (allocationEndTimeMap1.get(entry2.getKey()) <= mainEndTime1)) {
                                        newOverlapPenaltyValue = allocationEndTimeMap1.get(entry2.getKey())
                                                - allocationStartTimeMap1.get(entry2.getKey());
                                    } else if ((allocationStartTimeMap1.get(entry2.getKey()) <= mainStartTime1)
                                            && (allocationEndTimeMap1.get(entry2.getKey()) > mainStartTime1)) {
                                        newOverlapPenaltyValue = allocationEndTimeMap1.get(entry2.getKey()) - mainStartTime1;
                                    } else if ((allocationStartTimeMap1.get(entry2.getKey()) < mainEndTime1)
                                            && (allocationEndTimeMap1.get(entry2.getKey()) >= mainEndTime1)) {
                                        newOverlapPenaltyValue = mainEndTime1 - allocationStartTimeMap1.get(entry2.getKey());
                                    }

                                    if ((allocationStartTimeMap2.get(entry2.getKey()) <= mainStartTime2)
                                            && (allocationEndTimeMap2.get(entry2.getKey()) >= mainEndTime2)) {
                                        newOverlapPenaltyValue += mainEndTime2 - mainStartTime2;
                                    } else if ((allocationStartTimeMap2.get(entry2.getKey()) >= mainStartTime2)
                                            && (allocationEndTimeMap2.get(entry2.getKey()) <= mainEndTime2)) {
                                        newOverlapPenaltyValue += allocationEndTimeMap2.get(entry2.getKey())
                                                - allocationStartTimeMap2.get(entry2.getKey());
                                    } else if ((allocationStartTimeMap2.get(entry2.getKey()) <= mainStartTime2)
                                            && (allocationEndTimeMap2.get(entry2.getKey()) > mainStartTime2)) {
                                        newOverlapPenaltyValue += allocationEndTimeMap2.get(entry2.getKey()) - mainStartTime2;
                                    } else if ((allocationStartTimeMap2.get(entry2.getKey()) < mainEndTime2)
                                            && (allocationEndTimeMap2.get(entry2.getKey()) >= mainEndTime2)) {
                                    ...
                                    if (newOverlapPenaltyValue > 0L) {
                                        ...
                                        segmentOverlapMap.put(
                                                generateCompositeKey(entry1.getKey().toString(), entry2.getKey().toString()),
                                                newOverlapPenaltyValue);
                                        segmentOverlapMap.put(
                                                generateCompositeKey(entry2.getKey().toString(), entry1.getKey().toString()),
                                                newOverlapPenaltyValue);
                                    }
                                }
                            }
        ...
        if (COMPUTE_hard1Score) {
            hard1Score =
                    hard1Score - newOtherPenaltyValue - newCurrentPenaltyValue + oldOtherPenaltyValue + oldCurrentPenaltyValue;
        } else {
            hard1Score = 0;
        }
        ...
        for (Map.Entry<Long, Segment> entry1 : segmentMap.entrySet()) {
            ...
            if (mainSegment.getBatch().getId() == allocationPath.getBatch().getId()) {
                ...
                for (Map.Entry<Long, Segment> entry2 : segmentMap.entrySet()) {
                    ...
                    if ((otherSegment.getBatch().getId() != allocationPath.getBatch().getId())
                            && (otherSegment.getName().equals(mainSegment.getName())) && (segmentOverlapMap.get(
                                    generateCompositeKey(entry1.getKey().toString(), entry2.getKey().toString())) != null)) {
            ...
            if (allocation.getRoutePath().getPath().equals(batchRoutePathMap.get(allocation.getBatch().getId()))) {
                if ((allocation.getDelay() != null) && (allocationDelayMap.get(allocation.getSegment().getId()) == null)) {
                    ...
                    if (mainStartTime1 != null) {
                        for (Map.Entry<Long, Segment> entry2 : segmentMap.entrySet()) {
                            if ((allocationDelayMap.get(entry2.getKey()) != null)
                                    && (entry2.getValue().getBatch().getId() != allocation.getBatch().getId())
                                    && (batchRoutePathMap.get(entry2.getValue().getBatch().getId()) != null)
                                    && (entry2.getValue().getRoutePath().getPath()
                                            .equals(batchRoutePathMap.get(entry2.getValue().getBatch().getId())))
                                    && (entry2.getValue().getName().equals(allocation.getSegment().getName()))
                                    && (allocationStartTimeMap1.get(entry2.getKey()) != null)
                                    && (allocationEndTimeMap1.get(entry2.getKey()) != null)) {
                                Long newOverlapPenaltyValue = 0L;
    ...
    private void retract(Allocation allocation) {
        ...
        for (Map.Entry<Long, Segment> entry2 : segmentMap.entrySet()) {
            ...
            if ((otherSegment.getBatch().getId() != allocation.getBatch().getId())
                    && (otherSegment.getName().equals(allocation.getSegment().getName()))
                    && (segmentOverlapMap.get(generateCompositeKey(allocation.getSegment().getId().toString(),
                            entry2.getKey().toString())) != null)) {
                hard2Score += (2 * segmentOverlapMap
                        .get(generateCompositeKey(allocation.getSegment().getId().toString(), entry2.getKey().toString())));
                segmentOverlapMap
                        .remove(generateCompositeKey(allocation.getSegment().getId().toString(), entry2.getKey().toString()));
                segmentOverlapMap
                        .remove(generateCompositeKey(entry2.getKey().toString(), allocation.getSegment().getId().toString()));
            }
        }
    ...
    public void updateBatchEndDate(AllocationPath allocationPath) {
        ...
        for (Map.Entry<Long, Segment> entry2 : segmentMap.entrySet()) {
            if ((entry2.getValue().getBatch().getId() == allocationPath.getBatch().getId())
                    && (allocationEndTimeMap1.get(entry2.getKey()) != null)
                    && (batchRoutePathMap.get(entry2.getValue().getBatch().getId()) != null) && (entry2.getValue()
                            .getRoutePath().getPath().equals(batchRoutePathMap.get(entry2.getValue().getBatch().getId())))) {
