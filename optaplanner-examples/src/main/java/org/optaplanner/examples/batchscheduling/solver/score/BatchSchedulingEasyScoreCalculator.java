...
public class BatchSchedulingEasyScoreCalculator implements EasyScoreCalculator<Schedule, BendableLongScore> 
{
		...
		{	
			for (AllocationPath allocationPath : schedule.getAllocationPathList()) 
			{
				...
				{	
					...
					{
						if ((allocationPath2.getRoutePath() != null) && (!(allocationPath.getBatch().equals(allocationPath2.getBatch()))))
						{	
			...
			for (Allocation allocation : schedule.getAllocationList()) 
			{
				if (batchRoutePathMap.get(allocation.getBatch().getId()) != null)
				{
					...
					{	
						...
						{
							batchCurrentPenaltyValueMap.put(allocation.getBatch().getId(), batchCurrentPenaltyValueMap.get(allocation.getBatch().getId()) + 1);
						}
						else if (allocation.getStartInjectionTime() != null)
						{	
							for (Allocation allocation2 : schedule.getAllocationList()) 
							{
								if 	((allocation2.getDelay() != null) && (allocation2.getBatch().getId() != allocation.getBatch().getId()) && (batchRoutePathMap.get(allocation2.getBatch().getId()) != null) && (allocation2.getRoutePath().getPath().equals(batchRoutePathMap.get(allocation2.getBatch().getId()))) && (allocation2.getSegment().getName().equals(allocation.getSegment().getName())))
								{	
									Long newOverlapPenaltyValue = 0L;
									
									if ((allocation2.getStartInjectionTime() != null) && ( allocation2.getEndInjectionTime() != null))
									{	
										if ((allocation2.getStartInjectionTime() <= allocation.getStartInjectionTime()) && ( allocation2.getEndInjectionTime() >= allocation.getEndInjectionTime()))
										{
											newOverlapPenaltyValue = allocation.getEndInjectionTime() - allocation.getStartInjectionTime();
										} 
										else if ((allocation2.getStartInjectionTime() >= allocation.getStartInjectionTime()) && ( allocation2.getEndInjectionTime() <= allocation.getEndInjectionTime()))
										{
											newOverlapPenaltyValue =  allocation2.getEndInjectionTime() - allocation2.getStartInjectionTime();
										}	
										else if ((allocation2.getStartInjectionTime() <= allocation.getStartInjectionTime()) && ( allocation2.getEndInjectionTime() > allocation.getStartInjectionTime()))
										{
											newOverlapPenaltyValue =  allocation2.getEndInjectionTime() - allocation.getStartInjectionTime();
										}	
										else if ((allocation2.getStartInjectionTime() < allocation.getEndInjectionTime()) && ( allocation2.getEndInjectionTime() >= allocation.getEndInjectionTime()))
										{
											newOverlapPenaltyValue = allocation.getEndInjectionTime() - allocation2.getStartInjectionTime();
										}
										
										if ((allocation2.getStartDeliveryTime() <= allocation.getStartDeliveryTime()) && (allocation2.getEndDeliveryTime() >= allocation.getEndDeliveryTime()))
										{
											newOverlapPenaltyValue += allocation.getEndDeliveryTime() - allocation.getStartDeliveryTime();
										} 
										else if ((allocation2.getStartDeliveryTime() >= allocation.getStartDeliveryTime()) && (allocation2.getEndDeliveryTime() <= allocation.getEndDeliveryTime()))
										{
											newOverlapPenaltyValue += allocation2.getEndDeliveryTime() - allocation2.getStartDeliveryTime();
										}	
										else if ((allocation2.getStartDeliveryTime() <= allocation.getStartDeliveryTime()) && (allocation2.getEndDeliveryTime() > allocation.getStartDeliveryTime()))
										{
											newOverlapPenaltyValue += allocation2.getEndDeliveryTime() - allocation.getStartDeliveryTime();
										}	
										else if ((allocation2.getStartDeliveryTime() < allocation.getEndDeliveryTime()) && (allocation2.getEndDeliveryTime() >= allocation.getEndDeliveryTime()))
										{
											newOverlapPenaltyValue += allocation.getEndDeliveryTime() - allocation2.getStartDeliveryTime();
										}

										if ((allocation2.getStartInjectionTime() >= allocation.getStartInjectionTime()) && (allocation2.getEndDeliveryTime() <= allocation.getEndDeliveryTime()))
										{
											newOverlapPenaltyValue += allocation.getEndDeliveryTime() - allocation2.getEndDeliveryTime();
										} 

										if ((allocation2.getStartInjectionTime() <= allocation.getStartInjectionTime()) && (allocation2.getEndDeliveryTime() >= allocation.getEndDeliveryTime()))
										{
											newOverlapPenaltyValue += allocation2.getEndDeliveryTime() - allocation.getEndDeliveryTime();
										} 
										
										if (newOverlapPenaltyValue > 0L)
										{	
											segmentOverlapMap.put(generateCompositeKey(allocation.getSegment().getId().toString(),allocation2.getSegment().getId().toString()) , newOverlapPenaltyValue);
										}	
									}	
								}	
							}	
					...
					else
					{
						if (allocation.getDelay() != null)
						{
							batchOtherPenaltyValueMap.put(allocation.getBatch().getId(), batchOtherPenaltyValueMap.get(allocation.getBatch().getId()) + 1);
						}
					}
				}
				else
	...
	{
		return allocation.getBatch().getName() + ":" + allocation.getRoutePath().getPath() + ":" + allocation.getSegment().getName();
	}
