...
. Update the project version on the `development` branch to the next minor snapshot.
Update the optaplanner version via the `version.org.optaplanner` property in all quickstarts, including Gradle build files.
