...
** During the `solve()` method, the CPU power will max out until it returns (except in <<daemon,daemon mode>> or if your <<BestSolutionListener,BestSolutionListener>> writes the best solution to disk or the network).
