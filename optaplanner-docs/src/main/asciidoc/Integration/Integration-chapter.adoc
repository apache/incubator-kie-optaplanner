...
so it can find the classpath resources (the solver config, domain classes, etc.) in your jars.
...
so <<constraintStreams,Constraint Streams>> and <<droolsScoreCalculation,Drools score calculation>> doesn't work on Android and its dependencies need to be excluded.
