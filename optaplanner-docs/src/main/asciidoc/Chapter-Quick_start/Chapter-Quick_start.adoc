...
Using a domain model helps determine which classes are planning entities and which of their properties are planning variables. Using a domain model helps to simplify contraints, improve performance, and increase flexibility for future needs. 

Beginning with the <<domainModelingGuide,domain model>>:
...
Try it yourself. <<downloadAndRunTheExamples,Download and configure the examples in your preferred IDE.>>
