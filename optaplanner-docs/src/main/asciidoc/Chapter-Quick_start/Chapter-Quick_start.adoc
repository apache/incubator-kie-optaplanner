...
//procedure from chapter 17 - Design Patterns
...
// I think the link here to the Solver configuration section in the Planner Configuration chapter is confusing for the user, especially as it deals with a different example.
+
...
+
We need to make Planner aware of our domain classes. In this configuration, it will automatically scan all classes in your classpath (for an `@PlanningEntity` or `@PlanningSolution` annotation):
+
