...
If that filter should apply on all entities, configure it as a <<pinnedPlanningEntities,global pinningFilter>> instead.
...
[WARNING]
====
A custom `MoveListFactory` implementation must ensure that it does not move <<pinnedPlanningEntities, pinned entities>>.
====
...
[WARNING]
====
A custom `MoveIteratorFactory` implementation must ensure that it does not move <<pinnedPlanningEntities, pinned entities>>.
====
