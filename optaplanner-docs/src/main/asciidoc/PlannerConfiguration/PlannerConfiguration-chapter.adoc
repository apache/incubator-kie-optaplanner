...
[NOTE]
====
On some environments (<<integrationWithOSGi,OSGi>>, <<integrationWithJBossModules,JBoss modules>>, ...), classpath resources (the solver config, domain classes, etc.) in your jars might not be available to the default `ClassLoader` of the `optaplanner-core` jar.
...
Use <<constraintStreamsFrom,fromUnfiltered()>> to avoid such unwanted behaviour.
====
...
OptaPlanner exposes metrics via https://micrometer.io/[Micrometer] which can be used for monitoring the Solver. OptaPlanner automatically connect to configured registries when used in Quarkus or Spring Boot. In plain Java, add the registry to the global Metrics object:
[source,java,nowrap]
----
...
- `optaplanner.solver.solve-length.seconds-max`: How long the oldest active solver been running

- `optaplanner.solver.solve-length.seconds-duration-sum`: The sum of each active solver solve length (ex: if there are two active solvers, one been running for 3 minutes, the other for 1 minute, this would be 4 minutes).
