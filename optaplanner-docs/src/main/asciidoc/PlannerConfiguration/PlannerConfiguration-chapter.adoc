...
[NOTE]
====
Planning entity `hashCode()` implementations must never change their return values. Therefore entity `hashCode()` must not depend on any planning variables. Pay special attention when using data structures with auto-generated `hashCode()` as entities, such as Java records or Kotlin data classes. ```
====
