...
[NOTE]
====
In case you only need to act when a particular fact exists, and you don't need to access the fact itself, use
<<constraintStreamsConditionalPropagation,conditional propagation>> instead.
Conditional propagation does not create cartesian products and therefore generally performs better.
====
