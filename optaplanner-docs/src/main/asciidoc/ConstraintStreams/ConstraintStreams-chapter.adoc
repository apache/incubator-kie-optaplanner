...
Conversely, the `ifNotExists()` building block is available (as well as the `ifNotExistsOther()` building block on
`UniConstraintStream`) to achieve the opposite effect:
