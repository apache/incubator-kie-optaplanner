...
Constraint name::
The human readable descriptive name for the constraint, which must be unique within the entire `ConstraintProvider`
implementation.
Constraint weight::
...
The penalty or reward score impact is the constraint weight multiplied by the match weigh.
This number is determined by applying the match weigher function to the contents of the constraint stream.
The default value is `1`.
Constraint package (Optional)::
Java package that contains the constraint.
...
It first enumerate all constraint matches and then applies a predicate to filter some matches out.
...
[NOTE]
====
For performance reasons, using the <<constraintStreamsJoin,join>> building block with the appropriate `Joiner`s is
...
For a list of all supported `Joiner` implementations, refer to the `org.optaplanner.core.api.score.stream.Joiners` class.
