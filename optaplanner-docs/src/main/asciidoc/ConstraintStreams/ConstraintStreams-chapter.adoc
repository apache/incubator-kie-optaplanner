...
With the `given(...)` call, we enumerate all the facts that the constraint stream will operate on.
In this case, we only list the `queen1` and `queen2` instances previously created.
Alternatively, you can call a `givenSolution(...)` here and provide a planning solution instead.
...
Second, instead of either a `penalizesBy()` or `rewardsWith()` call, we use the `scores(...)` method.
This runs the `ConstraintProvider` on the given facts and returns a sum of `Score`s of all constraint matches resulting
from the given facts.

Using this method, you can make sure that the constraint provider does not miss any constraints and that the scoring
function remains consistent as you evolve your code base.
