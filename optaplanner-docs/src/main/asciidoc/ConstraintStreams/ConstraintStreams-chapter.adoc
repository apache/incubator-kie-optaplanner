...
[NOTE]
====
For performance reasons, using the <<constraintStreamsJoin,join>> building block with the appropriate `JoinerMutator` is preferrable when possible.
Using a `JoinerMutator` creates only the constraint matches that are necessary, while filtered join creates all possible constraint matches and only then filters some of them out.
====
...
Use `JoinerMutator` to restrict your joins only to the matches you are actually interested in, as shown in this example:
...
The following `JoinerMutator` types are supported:
...
For a full list of all supported `JoinerMutator` implementations and their various overloads, refer to the
...
Also note the use of the `JoinerMutator` class to limit the constraint matches.
...
[NOTE]
====
For performance reasons, using conditional propagation with the appropriate `JoinerMutator` instance is preferable to joining.
