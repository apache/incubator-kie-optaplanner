...
NOTE: Course scheduling is different because it uses two xref:planner-configuration/planner-configuration.adoc#planningVariable[planning variables].

NOTE: Vehicle routing is different because it uses a xref:planner-configuration/planner-configuration.adoc#planningListVariable[planning list variable].

NOTE: Traveling salesmen is different because it uses a xref:planner-configuration/planner-configuration.adoc#chainedPlanningVariable[chained planning variable].
...
However, the time coordinate of each element can be deduced from its order in the sequence.
If the elements’ position on time axis affects the score, use a xref:shadow-variable/shadow-variable.adoc#shadowVariable[shadow variable] to calculate the time.

This pattern is implemented using either the xref:planner-configuration/planner-configuration.adoc#chainedPlanningVariable[chained planning variable] or the xref:planner-configuration/planner-configuration.adoc#planningListVariable[planning list variable].
The two modeling approaches are equivalent because they both allow OptaPlanner to order elements in sequences of variable lengths.
The planning list variable is easier to use than the chained planning variable, but it does not support all the advanced planning techniques.

[[chainedThroughTimePatternChainedVariable]]
==== Chained through time pattern using chained planning variable

Using the xref:planner-configuration/planner-configuration.adoc#chainedPlanningVariable[chained planning variable], planning entities are arranged in a recursive data structure, forming a chain, that ends with an anchor.
...
The starting time of an entity is usually a xref:shadow-variable/shadow-variable.adoc#shadowVariable[shadow variable].
...
OptaPlanner distributes planning values into planning entities’ xref:planner-configuration/planner-configuration.adoc#planningListVariable[planning list variable].
...
It lasts 52 minutes, therefore her second task starts at 8:52.
The starting time of an element is usually a xref:shadow-variable/shadow-variable.adoc#shadowVariable[shadow variable].
