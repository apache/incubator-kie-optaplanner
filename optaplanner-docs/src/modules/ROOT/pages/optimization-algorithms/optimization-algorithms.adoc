...
Configuring this termination on a specific `Phase` (such as ``<localSearch>``) instead of on the `Solver` itself is often a better option.
...
To do that, read about xref:repeated-planning/repeated-planning.adoc#repeatedPlanning[repeated planning] and do it with a xref:repeated-planning/repeated-planning.adoc#problemFactChange[ProblemFactChange] instead.
====
