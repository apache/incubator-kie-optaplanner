...
`SolutionManager` (formerly `ScoreManager`) received a new method called `triggerVariableListeners()`.
This method has often been requested by users who load their solutions from persistent storage (such as a relation database)
and these solutions do not carry the information carried in xref:shadow-variable/shadow-variable.adoc[shadow variables].
By calling it,
OptaPlanner will automatically compute values for all the shadow variables
in the given solution.

`ProblemChangeDirector` also received a new method of the same name,
