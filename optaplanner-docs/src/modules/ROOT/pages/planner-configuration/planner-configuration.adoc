...
[NOTE]
====
On some environments (xref:integration/integration.adoc#integrationWithOSGi[OSGi], xref:integration/integration.adoc#integrationWithJBossModules[JBoss modules], ...), classpath resources (the solver config, domain classes, etc.) in your jars might not be available to the default `ClassLoader` of the `optaplanner-core` jar.
...
Use xref:constraint-streams/constraint-streams.adoc#constraintStreamsFrom[fromUnfiltered()] to avoid such unwanted behaviour.
====
