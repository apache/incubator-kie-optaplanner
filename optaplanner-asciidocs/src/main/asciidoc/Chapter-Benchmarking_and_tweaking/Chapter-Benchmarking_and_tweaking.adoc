...
[[_findthebestsolverconfiguration]]
...
The `inputSolutionFile` is read by a <<_solutionfileio,SolutionFileIO>> (relative to the working directory).
...
[[_readinganinputsolutionfromadatabase]]
...
* For each dataset, create a txt file that holds the unique id of the dataset. Write <<_customsolutionfileio,a custom `SolutionFileIO`>> that reads that identifier, connects to the database and extract the problem identified by that id. Configure those txt files as `<inputSolutionFile>` elements.
...
[[_writetheoutputsolutionofbenchmarkruns]]
...
Benchmark logging is configured like <<_logging,solver logging>>.
...
[[_benchmarkreportsummarystatistics]]
== Summary Statistics

[[_benchmarkreportbestscoresummary]]
...
[[_benchmarkreportbestscorescalabilitysummary]]
...
[[_benchmarkreportbestscoredistributionsummary]]
...
Enable <<_statisticalbenchmarking,statistical benchmarking>> to use this summary.

[[_benchmarkreportwinningscoredifferencesummary]]
...
[[_benchmarkreportworstscoredifferencepercentagesummary]]
...
[[_benchmarkreportscorecalculationspeedsummary]]
...
[[_benchmarkreporttimespentsummary]]
...
[[_benchmarkreporttimespentscalabilitysummary]]
...
[[_benchmarkreportbestscorepertimespentsummary]]
...
[[_benchmarkreportstatisticperdataset]]
== Statistic Per Dataset (Graph And CSV)

[[_enableaproblemstatistic]]
...
[[_benchmarkreportbestscoreovertimestatistic]]
...
potential <<_scoretrap,score trap>> which gets the solver temporarily stuck in a local
optima.*
...
[[_benchmarkreportstepscoreovertimestatistic]]
...
[[_benchmarkreportscorecalculationspeedtatistic]]
...
[[_benchmarkreportbestsolutionmutationovertimestatistic]]
...
[[_benchmarkreportmovecountperstepstatistic]]
...
[[_benchmarkreportmemoryusestatistic]]
...
[[_benchmarkreportstatisticpersinglebenchmark]]
== Statistic Per Single Benchmark (Graph And CSV)

[[_enableasinglestatistic]]
...
[[_benchmarkreportconstraintmatchtotalbestscoreovertimestatistic]]
...
Requires the score calculation to support <<_explainingthescore,constraint matches>>. <<_droolsscorecalculation,Drools score calculation>> supports constraint matches automatically, but <<_incrementaljavascorecalculation,incremental Java score calculation>> requires more work.
...
[[_benchmarkreportconstraintmatchtotalstepscoreovertimestatistic]]
...
[[_benchmarkreportpickedmovetypebestscorediffovertimestatistic]]
...
[[_benchmarkreportpickedmovetypestepscorediffovertimestatistic]]
...
[[_advancedbenchmarking]]
== Advanced Benchmarking

[[_benchmarkingperformancetricks]]
=== Benchmarking Performance Tricks

[[_parallelbenchmarkingonmultiplethreads]]
...
Each individual result is also visible in the report, as well as plotted in <<_benchmarkreportbestscoredistributionsummary,the best score distribution summary>>.

Just add a `<subSingleCount>` element to an <<_inheritedsolverbenchmark,`<inheritedSolverBenchmark>`>> element or in a `<solverBenchmark>` element:
...
[NOTE]
====
If `subSingleCount` is higher than ``1``, the benchmarker will automatically use a _different_<<_randomnumbergenerator,`Random` seed>> for every sub single run, without losing reproducibility (for each sub single index) in <<_environmentmode,EnvironmentMode>>``REPRODUCIBLE`` and lower.
====

[[_templatebasedbenchmarking]]
