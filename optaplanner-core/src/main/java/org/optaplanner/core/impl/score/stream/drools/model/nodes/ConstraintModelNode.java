...
public interface ConstraintModelNode {
    ...
    static <A> UniConstraintModelChildNode<A> filter(Predicate<A> predicate) {
        return new UniFilterNode<>(predicate);
    }

    static <A, B> BiConstraintModelNode<A, B> filter(BiPredicate<A, B> predicate) {
        return new BiFilterNode<>(predicate);
    }

    static <A, B, C> TriConstraintModelNode<A, B, C> filter(TriPredicate<A, B, C> predicate) {
        return new TriFilterNode<>(predicate);
    }

    static <A, B, C, D> QuadConstraintModelNode<A, B, C, D> filter(QuadPredicate<A, B, C, D> predicate) {
        return new QuadFilterNode<>(predicate);
    }

    static <A, B> BiConstraintModelNode<A, B> join(Class<B> otherFactType, AbstractBiJoiner<A, B> joiner) {
        return new BiJoinNode<>(otherFactType, joiner);
    }

    static <A, B, C> TriConstraintModelNode<A, B, C> join(Class<C> otherFactType, AbstractTriJoiner<A, B, C> joiner) {
        return new TriJoinNode<>(otherFactType, joiner);
    }

    static <A, B, C, D> QuadConstraintModelNode<A, B, C, D> join(Class<D> otherFactType,
            AbstractQuadJoiner<A, B, C, D> joiner) {
        return new QuadJoinNode<>(otherFactType, joiner);
    }

    static <A, B> UniConstraintModelChildNode<A> ifExists(Class<B> otherFactType, AbstractBiJoiner<A, B> joiner) {
        return new UniExistenceNode<>(otherFactType, joiner, true);
    }

    static <A, B, C> BiConstraintModelNode<A, B> ifExists(Class<C> otherFactType, AbstractTriJoiner<A, B, C> joiner) {
        return new BiExistenceNode<>(otherFactType, joiner, true);
    }

    static <A, B, C, D> TriConstraintModelNode<A, B, C> ifExists(Class<D> otherFactType,
            AbstractQuadJoiner<A, B, C, D> joiner) {
        return new TriExistenceNode<>(otherFactType, joiner, true);
    }

    static <A, B, C, D, E> QuadConstraintModelNode<A, B, C, D> ifExists(Class<E> otherFactType,
            AbstractPentaJoiner<A, B, C, D, E> joiner) {
        return new QuadExistenceNode<>(otherFactType, joiner, true);
    }

    static <A, B> UniConstraintModelChildNode<A> ifNotExists(Class<B> otherFactType, AbstractBiJoiner<A, B> joiner) {
        return new UniExistenceNode<>(otherFactType, joiner, false);
    }

    static <A, B, C> BiConstraintModelNode<A, B> ifNotExists(Class<C> otherFactType,
            AbstractTriJoiner<A, B, C> joiner) {
        return new BiExistenceNode<>(otherFactType, joiner, false);
    }

    static <A, B, C, D> TriConstraintModelNode<A, B, C> ifNotExists(Class<D> otherFactType,
            AbstractQuadJoiner<A, B, C, D> joiner) {
        return new TriExistenceNode<>(otherFactType, joiner, false);
    }

    static <A, B, C, D, E> QuadConstraintModelNode<A, B, C, D> ifNotExists(Class<E> otherFactType,
            AbstractPentaJoiner<A, B, C, D, E> joiner) {
        return new QuadExistenceNode<>(otherFactType, joiner, false);
    }

    static <A, GroupKey_> UniConstraintModelChildNode<GroupKey_> groupBy(Function<A, GroupKey_> mapping) {
        return new UniToUniGroupingNode<>(mapping);
    }

    static <A, GroupKeyA_, GroupKeyB_> BiConstraintModelNode<GroupKeyA_, GroupKeyB_> groupBy(
            Function<A, GroupKeyA_> aMapping, Function<A, GroupKeyB_> bMapping) {
        return new UniToBiGroupingNode<>(aMapping, bMapping);
    }

    static <A, ResultContainer_, Result_> UniConstraintModelChildNode<Result_> groupBy(
            UniConstraintCollector<A, ResultContainer_, Result_> collector) {
        return new UniToUniGroupingNode<>(collector);
    }

    static <A, GroupKey_, ResultContainer_, Result_> BiConstraintModelNode<GroupKey_, Result_> groupBy(
    ...
    static <A, GroupKeyA_, GroupKeyB_, ResultContainer_, Result_>
            TriConstraintModelNode<GroupKeyA_, GroupKeyB_, Result_> groupBy(Function<A, GroupKeyA_> aMapping,
    ...
    static <A, GroupKeyA_, GroupKeyB_, ResultContainerC_, ResultC_, ResultContainerD_, ResultD_>
            QuadConstraintModelNode<GroupKeyA_, GroupKeyB_, ResultC_, ResultD_> groupBy(Function<A, GroupKeyA_> aMapping,
    ...
    static <A, B, ResultContainer_, Result_> UniConstraintModelChildNode<Result_> groupBy(
            BiConstraintCollector<A, B, ResultContainer_, Result_> collector) {
        return new BiToUniGroupingNode<>(collector);
    }

    static <A, B, GroupKey_> UniConstraintModelChildNode<GroupKey_> groupBy(BiFunction<A, B, GroupKey_> mapping) {
        return new BiToUniGroupingNode<>(mapping);
    }

    static <A, B, GroupKey_, ResultContainer_, Result_> BiConstraintModelNode<GroupKey_, Result_> groupBy(
    ...
    static <A, B, GroupKeyA_, GroupKeyB_> BiConstraintModelNode<GroupKeyA_, GroupKeyB_> groupBy(
    ...
    static <A, B, GroupKeyA_, GroupKeyB_, ResultContainer_, Result_>
            TriConstraintModelNode<GroupKeyA_, GroupKeyB_, Result_> groupBy(BiFunction<A, B, GroupKeyA_> aMapping,
    ...
    static <A, B, GroupKeyA_, GroupKeyB_, ResultContainerC_, ResultC_, ResultContainerD_, ResultD_>
            QuadConstraintModelNode<GroupKeyA_, GroupKeyB_, ResultC_, ResultD_> groupBy(BiFunction<A, B, GroupKeyA_> aMapping,
    ...
    static <A, B, C, ResultContainer_, Result_> UniConstraintModelChildNode<Result_> groupBy(
            TriConstraintCollector<A, B, C, ResultContainer_, Result_> collector) {
        return new TriToUniGroupingNode<>(collector);
    }

    static <A, B, C, GroupKey_> UniConstraintModelChildNode<GroupKey_> groupBy(
            TriFunction<A, B, C, GroupKey_> mapping) {
        return new TriToUniGroupingNode<>(mapping);
    }

    static <A, B, C, GroupKey_, ResultContainer_, Result_> BiConstraintModelNode<GroupKey_, Result_> groupBy(
            TriFunction<A, B, C, GroupKey_> mapping,
            TriConstraintCollector<A, B, C, ResultContainer_, Result_> collector) {
        return new TriToBiGroupingNode<>(mapping, collector);
    }

    static <A, B, C, GroupKeyA_, GroupKeyB_> BiConstraintModelNode<GroupKeyA_, GroupKeyB_> groupBy(
    ...
    static <A, B, C, GroupKeyA_, GroupKeyB_, ResultContainer_, Result_>
            TriConstraintModelNode<GroupKeyA_, GroupKeyB_, Result_> groupBy(TriFunction<A, B, C, GroupKeyA_> aMapping,
    ...
    static <A, B, C, D, ResultContainer_, Result_> UniConstraintModelChildNode<Result_> groupBy(
            QuadConstraintCollector<A, B, C, D, ResultContainer_, Result_> collector) {
        return new QuadToUniGroupingNode<>(collector);
    }

    static <A, B, C, D, GroupKey_> UniConstraintModelChildNode<GroupKey_> groupBy(
            QuadFunction<A, B, C, D, GroupKey_> mapping) {
        return new QuadToUniGroupingNode<>(mapping);
    }

    static <A, B, C, D, GroupKey_, ResultContainer_, Result_> BiConstraintModelNode<GroupKey_, Result_> groupBy(
            QuadFunction<A, B, C, D, GroupKey_> mapping,
            QuadConstraintCollector<A, B, C, D, ResultContainer_, Result_> collector) {
        return new QuadToBiGroupingNode<>(mapping, collector);
    }

    static <A, B, C, D, GroupKeyA_, GroupKeyB_> BiConstraintModelNode<GroupKeyA_, GroupKeyB_> groupBy(
    ...
    static <A, B, C, D, GroupKeyA_, GroupKeyB_, ResultContainer_, Result_>
            TriConstraintModelNode<GroupKeyA_, GroupKeyB_, Result_> groupBy(QuadFunction<A, B, C, D, GroupKeyA_> aMapping,
     ...
     */
    List<ConstraintModelNode> getChildNodes();
