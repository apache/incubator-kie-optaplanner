...
final class BiExistenceMutator<A, B, C> implements Mutator {
    ...
    public AbstractRuleAssembler apply(AbstractRuleAssembler ruleAssembler) {
        if (shouldExist) {
            return new BiRuleAssembler(((BiLeftHandSide) ruleAssembler.leftHandSide).exists(otherFactType, joiners));
        } else {
            return new BiRuleAssembler(((BiLeftHandSide) ruleAssembler.leftHandSide).notExists(otherFactType, joiners));
        }
    }
