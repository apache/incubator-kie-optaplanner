...
final class TriExistenceMutator<A, B, C, D> implements Mutator {
    ...
    public AbstractRuleAssembler apply(AbstractRuleAssembler ruleAssembler) {
        if (shouldExist) {
            return new TriRuleAssembler(((TriLeftHandSide) ruleAssembler.leftHandSide).exists(otherFactType, joiners));
        } else {
            return new TriRuleAssembler(((TriLeftHandSide) ruleAssembler.leftHandSide).notExists(otherFactType, joiners));
        }
    }
