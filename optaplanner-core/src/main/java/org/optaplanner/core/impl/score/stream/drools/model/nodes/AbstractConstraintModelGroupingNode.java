...
public abstract class AbstractConstraintModelGroupingNode<FunctionType_, CollectorType_>
    ...
    private static ConstraintModelNodeType determineType(List mappings, List collectors) {
        if (mappings.isEmpty()) {
            if (collectors.isEmpty()) {
                throw new IllegalStateException("Grouping node has no mappings or collectors.");
            } else {
                return ConstraintModelNodeType.GROUPBY_COLLECTING_ONLY;
            }
        } else if (collectors.isEmpty()) {
            return ConstraintModelNodeType.GROUPBY_MAPPING_ONLY;
        } else {
            return ConstraintModelNodeType.GROUPBY_MAPPING_AND_COLLECTING;
        }
    }
