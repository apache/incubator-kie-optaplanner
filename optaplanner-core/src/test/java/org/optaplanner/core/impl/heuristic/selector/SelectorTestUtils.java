...
public class SelectorTestUtils {
    ...
    public static EntitySelector mockEntitySelector(EntityDescriptor entityDescriptor,
        ...
        when(entitySelector.iterator()).thenAnswer((Answer<Iterator<Object>>) invocation -> entityList.iterator());
        when(entitySelector.listIterator()).thenAnswer((Answer<ListIterator<Object>>) invocation -> entityList.listIterator());
        for (int i = 0; i < entityList.size(); i++) {
            final int index = i;
            when(entitySelector.listIterator(index)).thenAnswer((Answer<ListIterator<Object>>) invocation -> entityList.listIterator(index));
        }
        when(entitySelector.endingIterator()).thenAnswer((Answer<Iterator<Object>>) invocation -> entityList.iterator());
    ...
    public static ValueSelector mockValueSelector(GenuineVariableDescriptor variableDescriptor, Object... values) {
        ...
        when(valueSelector.iterator(any())).thenAnswer((Answer<Iterator<Object>>) invocation -> valueList.iterator());
    ...
    public static ValueSelector mockValueSelectorForEntity(GenuineVariableDescriptor variableDescriptor,
        ...
        for (Map.Entry<Object, Collection<Object>> entry : entityToValues.asMap().entrySet()) {
            ...
            when(valueSelector.getSize(entity)).thenAnswer((Answer<Long>) invocation -> (long) valueList.size());
            when(valueSelector.iterator(entity)).thenAnswer((Answer<Iterator<Object>>) invocation -> valueList.iterator());
    ...
    public static EntityIndependentValueSelector mockEntityIndependentValueSelector(
        ...
        final List<Object> valueList = Arrays.<Object>asList(values);
        when(valueSelector.iterator(any())).thenAnswer((Answer<Iterator<Object>>) invocation -> valueList.iterator());
        when(valueSelector.endingIterator(any())).thenAnswer((Answer<Iterator<Object>>) invocation -> valueList.iterator());
        when(valueSelector.iterator()).thenAnswer((Answer<Iterator<Object>>) invocation -> valueList.iterator());
        when(valueSelector.isCountable()).thenReturn(true);
        when(valueSelector.isNeverEnding()).thenReturn(false);
        when(valueSelector.getSize(any())).thenReturn((long) valueList.size());
    ...
    public static MoveSelector mockMoveSelector(Class moveClass,
        ...
        when(moveSelector.iterator()).thenAnswer((Answer<Iterator<Move>>) invocation -> moveList.iterator());
