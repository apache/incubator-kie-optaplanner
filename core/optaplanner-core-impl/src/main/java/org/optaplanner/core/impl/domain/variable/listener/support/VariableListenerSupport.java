...
public final class VariableListenerSupport<Solution_> implements SupplyManager {
    ...
    private final Object2LongMap<Demand<?>> demandCounterMap = new Object2LongOpenHashMap<>();
    ...
    public <Supply_ extends Supply> Supply_ demand(Demand<Supply_> demand) {
        long activeDemandCount = incrementDemandCounter(demand);
        if (activeDemandCount == 1) { // This is a new demand, create the supply.
    ...
    private <Supply_ extends Supply> long incrementDemandCounter(Demand<Supply_> demand) {
        long currentCount = demandCounterMap.getLong(demand);
        long newCount = currentCount + 1;
        demandCounterMap.put(demand, newCount);
        return newCount;
    }
    ...
    public <Supply_ extends Supply> boolean cancel(Demand<Supply_> demand) {
        long newCount = decrementDemandCounter(demand);
        if (newCount == -1) {
            return false;
        } else if (newCount == 0) {
    ...
    private <Supply_ extends Supply> long decrementDemandCounter(Demand<Supply_> demand) {
        long currentCount = demandCounterMap.getLong(demand);
        if (currentCount < 1) {
            return -1;
        } else if (currentCount == 1) {
            demandCounterMap.removeLong(demand);
            return 0;
        } else {
            long newCount = currentCount - 1;
            demandCounterMap.put(demand, newCount);
    ...
    public <Supply_ extends Supply> long getActiveCount(Demand<Supply_> demand) {
        return demandCounterMap.getLong(demand);
    }
